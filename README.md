...
5. **Install the `python-dotenv` Library** (if you haven't already):
   ```sh
   pip install python-dotenv
   ```

6. **Modify the Python Script to Load Environment Variables**:
   - Update the `send_utxos.py` script to load environment variables from the `.env` file:
